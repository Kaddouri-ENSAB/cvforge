// src/components/analysis/CompletenessScore.tsx (Personne 2 — UI Polish)
...
function useScoreItems(): ScoreItem[] {
  ...
  return [
    { label: 'Nom complet',          done: !!personal.fullName.trim(),                          weight: 15 },
    { label: 'Email',                done: !!personal.email.trim(),                             weight: 10 },
    { label: 'Téléphone',           done: !!personal.phone.trim(),                             weight: 10 },
    { label: 'Résumé professionnel', done: !!personal.summary.trim(),                           weight: 15 },
    { label: 'Formation',            done: education.length > 0,                                weight: 15 },
    { label: 'Expérience / Projet',  done: experiences.length > 0 || projects.length > 0,      weight: 20 },
    { label: 'Compétences (min. 3)', done: skills.length >= 3,                                 weight: 15 },
  ];
}
    ...
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      <div className={`flex items-center gap-6 px-5 py-5 ${theme.bg}`}>
        ...
        <div className="flex-shrink-0">
          <svg
            ...
            height="130"
            viewBox="0 0 130 130"
            aria-label={`Score de complétude : ${score} sur 100`}
            role="img"
          >
            {/* Fond */}
            ...
            <text x="65" y="60" textAnchor="middle" fontSize="24" fontWeight="800"
              fill={theme.color} fontFamily="sans-serif">
              {score}%
            </text>
            {/* Label */}
      ...
      <ul className="divide-y divide-slate-50 px-5 py-2">
        {items.map((item) => (
          <li
            key={item.label}
            className="flex items-center gap-3 py-2.5 group hover:bg-slate-50 -mx-5 px-5 rounded transition-colors"
          >
            {/* Icône */}
            <span
              className={`
